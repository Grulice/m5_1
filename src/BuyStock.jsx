      ...
      fetch("https://5e8da89e22d8cd0016a798db.mockapi.io/users/1/stocks", {
        ...
        if (res.status === 200 || res.status === 201) {
          ...
          fetch("https://5e8da89e22d8cd0016a798db.mockapi.io/users/1", {
            ...
            // Поменяли логику обновления баланса
            // Новый баланс не рендерится графически, он заново запрашивается с сервера
            // Это сделано из-за того, что запрос на смену баланса может не дойти до сервера, а
            // у пользователя на интерфейсе будет отображаться так, будто он дошёл
            // Чтобы это предотвратить - баланс запрашивается с сервера после каждого обновления
  ...
  render() {
    ...
    return (
      <BuyStockBlock>
        ...
        <СountStocksBlock>
          <MinusOutlined
            onClick={() => this.handleClickChangeCountStocks(-1)}
            style={countStocksButtonStyle}
          />
        ...
        <TotlaPriceStoks>
          Buy for <Decimal number={result} />
        </TotlaPriceStoks>
        <SubmitButton to={"/Stock"}>
          <Button
            onClick={() => {
              this.sendInfoToApi(result);
            }}
            type="primary"
            shape="round"
            size="large"
